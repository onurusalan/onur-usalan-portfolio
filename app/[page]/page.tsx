...
export async function generateMetadata({ params }: { params: Promise<{ page: string }> }): Promise<Metadata> {
  ...
  return isPageType(page) ? staticMetadata(page, "en") : {};
}

export default async function EnglishStaticRoute({ params }: { params: Promise<{ page: string }> }) {
  const { page } = await params;
  if (!isPageType(page)) notFound();
  return <Page lang="en" type={page} />;
}
